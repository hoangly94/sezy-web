...
function* fetchTableSaga({ payload: { page = '1', code, direction, timestamp } }) {
    ...
    timestamp
        && timestamp !== 'all'
        && (() => {
            params.append('filter', `timestamp||$gt||${moment(timestamp).format(dateFormat)}`);
            params.append('filter', `timestamp||$lt||${moment(timestamp).add(1, 'd').format(dateFormat)}`);
        })();
    ...
    !timestamp
        ...
        && (() => {
            params.append('filter', `timestamp||$gt||${moment(filtersState.timestamp.value).format(dateFormat)}`);
            params.append('filter', `timestamp||$lt||${moment(filtersState.timestamp.value).add(1, 'd').format(dateFormat)}`);
        })();
